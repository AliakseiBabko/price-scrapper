...
### 11.1 ⚠️ Downdraft / In-Worktop Hoods Are a MATERIALS Decision, Not Only a Layout One (PRIDE / Алёна, 2026-09-06)

**§11's taxonomy lists worktop-integrated and cooktop-integrated (downdraft) units and prices their cost and planning burden. This adds the consequence for the surrounding kitchen, which is a separate constraint and is decided by the facade spec rather than by the hood.**

**Stated mechanism**: because extraction happens at worktop level rather than overhead, steam, grease and moisture ("пар, жир, влага") travel very close to the facades and the working surface instead of being drawn up and away from them. Moisture- and heat-sensitive kitchen materials therefore degrade over time. **Film-wrapped facades ("фасады в плёнке") are named as the worst case** — which maps directly onto the film-on-MDF mid tier on [[03_Kitchen/analysis/Furniture_Facade_Materials|Kitchen Facade Materials]].

- **⚠️ Stated selection rule — judge this hood type against three inputs together, not on appearance: (1) the visual result, (2) the kitchen's facade and worktop materials, and (3) how actively the household actually cooks.** Where cooking is heavy, a conventional overhead hood is called "намного надёжнее и проще в уходе" (more reliable and simpler to maintain).
- **⚠️ Third distinct cost now recorded for this hood family, and they are cumulative**: §11 gives the cabinetry/planning burden (non-standard cabinetry, must be designed in, not retrofitted); [[03_Kitchen/analysis/Appliance_Integration|Appliance Integration]] gives the hard sequencing dependency (floor-level duct routed *before* the screed pour); this gives the facade-material constraint. **Anyone specifying a downdraft unit for this flat has to clear all three**, and only the first is visible from a showroom.
- Note this is a *different* objection from the §2 capacity cap — it concerns what the extracted air passes on its way out, not how much of it the shaft can take.

`single-account`, `unverified`. Source is a furniture-procurement company with no stake in hood selection. [source: [[_Sources/YT_8uvwTALKbYE_pride_design_project_impractical_choices|YT_8uvwTALKbYE]]]
