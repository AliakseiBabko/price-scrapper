...
| `tools/layout/overlay_dxf_on_raster.py` | **Run after every change to the v0 DXF.** Registers the export onto `fllor_plan_detailed.jpeg` on WALL LINES (not the ink bounding box, which gave 6.6% anisotropy) and reports, per wall, the distance in mm from each sampled edge to drawn ink. A hit/miss probe is useless here — at ~13 mm/px almost any point has ink within a few pixels |
| `tools/layout/check_dxf_closure.py` | **THE closure gate. Run after every v0 export.** Rasterises the wall union and asserts: every ledger corner square is solid, no unsanctioned overlap, no unexplained cavity, every wall present or explicitly quarantined. Needs no raster, so it cannot be circular. Exits non-zero on breach |
| `scripts/dxf_closure_selftest.py` | Guards the gate: the real export must pass and six seeded defects must each be rejected — three corner voids, an overlap, a deleted wall, a global shift |
...
10. **A role is a claim, so it needs someone to have looked.** In `_Precedents/`, a file lands `examined: no` / `role: unexamined`, and **a layout case may not cite an unexamined file** — `tools/precedents/validate_precedents.py` enforces both. This exists because an unfurnished plan was filed as the developer handover state when it is the **after** state of a demolition. A confident label on an unlooked-at file is worse than no label.
11. **Merge to main every time.** After commit and push on a branch: merge `--no-ff` into main, push main, delete the branch locally and remotely. No lingering branches, no PRs.
