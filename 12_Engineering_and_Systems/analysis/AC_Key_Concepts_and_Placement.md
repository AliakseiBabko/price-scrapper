...
attribution: unconfirmed — these three placement rules are presented in the detail page without a cited extraction note whose `channel:` field isolates them: 1. **Don't blow cold air directly on occupants** — position the unit so it cools the room evenly instead of aiming a cold stream at where people actually sit/sleep.
attribution: unconfirmed — source channel not isolated for this numbered claim: 2. **Leave clear space above and below the indoor unit** — it needs unobstructed airflow to draw in warm air and release cooled air; boxing it in tightly (e.g. inside a cabinet with no clearance) defeats this.
attribution: unconfirmed — source channel not isolated for this numbered claim: 3. **Position it to be visually unobtrusive** — avoid the center of a wall or a spot that disrupts the room's sightlines/design where a less prominent option exists.
