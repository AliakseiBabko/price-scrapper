...
attribution: unconfirmed — the cited buying source is not channel-isolated: **Buy finish-stage switches/outlets/frames roughly one week before installation, not months in advance** — reduces storage damage/loss risk and avoids having already-bought fixtures invalidated by a mid-project design change. Count the actual number of finish electrical points room-by-room after rough-in is complete (not from the original drawing), since e.g. kitchen point counts commonly shift once final cabinetry is confirmed. Keep a small spare stock — roughly 3-4 spare outlets and a few spare switches/frames of each type used, for a ~100 m² apartment.

attribution: unconfirmed — the cited buying source is not channel-isolated: **Strip cable insulation only with a dedicated wire-stripping tool, never a utility/box knife** — a utility knife risks nicking the conductor's copper strands, a latent failure point. Remove a device's factory-installed mounting screws before rough-finish work (plastering/puttying), not at final finish-electrical install — screw holes left in during finishing get clogged with paint/putty and have to be re-drilled later.

attribution: unconfirmed — the cited buying source is not channel-isolated: **Lighting-circuit wiring is thinner-gauge than socket-circuit wiring and isn't rated for high-draw loads** — improvising a power tap from a light fixture/socket to run a higher-draw tool or appliance risks overheating the lighting circuit's conductors.
