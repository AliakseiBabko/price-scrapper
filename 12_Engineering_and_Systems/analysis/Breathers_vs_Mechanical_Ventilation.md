# Breathers vs. Full Mechanical Ventilation Systems

The equipment decision: a per-room breather against a whole-apartment mechanical system, and what each actually delivers. **Split out on 2026-09-02 under the vault's 300-line page ceiling, verbatim.**
...
- **The premise: «ВЛАЖНОСТЬ РАЗРУШАЕТ МЕБЕЛЬ И ОТДЕЛКУ.»**
- **⚠️ THE OBSERVATION, which is the useful half: «если вы когда-нибудь были в квартире, где организована ПРИТОЧНО-ВЫТЯЖНАЯ вентиляция, ИЗОЛИРОВАННАЯ ОТ УЛИЦЫ, вы обратите внимание на тот момент, что НА ПОЛУ НИГДЕ НЕТ ПЫЛИ. Вся пыль оседает в ФИЛЬТРАХ вентиляции, воздух постоянно меняется, и квартира очень чистая. ТАМ НУЖНА МИНИМАЛЬНАЯ УБОРКА.»**
- **⚠️⚠️ AND THE CHAIN THAT MAKES IT A DURABILITY CLAIM: the same practitioner argues that CLEANING IS ITSELF A WEAR MECHANISM — «чем ЧАЩЕ вы убираетесь, тем БЫСТРЕЕ вы РАЗРУШАЕТЕ интерьер». If a filtered supply system removes most of the dust before it lands, it removes most of the wiping, and therefore most of the wear that wiping causes.**
  **→ So an MVHR system's payback is not only air quality and heat recovery — it is a slower rate of finish degradation across the whole flat. That reframes it against the price ladders above, which price it purely as a comfort purchase.** Full framework on [[17_Design_and_Ergonomics/analysis/Practitioner_Material_Selection_Accounts|Practitioners' Own Material-Selection Accounts]]. `single-account` for the durability inference; the dust-in-filters observation is ordinary and checkable.
- **⚠️ The minimum, where a full system is out of scope — and it is stated as a handover check rather than a purchase: «даже если у вас НЕТ специальной приточно-вытяжной, то у вас ОБЯЗАТЕЛЬНО должна быть МЕХАНИЧЕСКАЯ вентиляция, которую вам делает ЗАСТРОЙЩИК. ОБЯЗАТЕЛЬНО ПРОВЕРЯЙТЕ этот момент.»** Consistent with the mistakes section above; the addition is that it belongs on an acceptance checklist.
- **⚠️ A high-humidity datum from his Dubai site work, offered as the extreme case: «у них ОЧЕНЬ БОЛЬШАЯ ВЛАЖНОСТЬ. Если вы выключили КОНДИЦИОНЕР на месяц — даже не на месяц, НА НЕДЕЛЮ — то по возвращению у вас, скорее всего, ВСЯ КВАРТИРА УЖЕ БУДЕТ ПОКРЫТА ПЛЕСЕНЬЮ. Это просто переделка всех стен.»** Reported from conversations with builders there.
  > [!NOTE]
