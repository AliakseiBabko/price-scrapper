# Lighting — Fixture Types, Chandeliers and Linear Lighting

What to check before buying a fitting, chandeliers and pendants, and the construction detail behind LED strip and linear lighting. **Split out on 2026-09-02 under the vault's 300-line page ceiling, verbatim.**
...
**⚠️ A fourth channel independently reaches the same centralisation rule, states the ventilation requirement as the reason, and adds one step nobody else does — LABEL THEM.** Designer Александр Синчуков: drivers «обязательно должны быть ВЕНТИЛИРУЕМЫЕ, чтобы они не задыхались. **Но если вы не хотите ПЕРЕДЕЛЫВАТЬ ВЕСЬ ПОТОЛОК в поиске вашего блока питания, то можете сделать ОТДЕЛЬНЫЙ ЩИТ для блоков** — чтобы они вентилировались, чтобы они все были в одном месте. Если что-то с ним случается, вы просто МЕНЯЕТЕ его.» So: **the same conclusion, the same overheating mechanism, and the same "don't hunt for it in the ceiling" argument, from a third unrelated practitioner.**

- **⚠️ AND THE STEP THE OTHER SOURCES OMIT, learned the hard way: «обязательно все их ПОДПИШИТЕ, потому что я недавно добавлял к треку в коридоре ДИММЕР, и мне пришлось ПЕРЕБРАТЬ ВСЕ треки, потому что они были тупо НЕ ПОДПИСАНЫ.»** A consolidated driver panel without labels converts a five-minute change into a full identification exercise — which is precisely the maintenance cost centralisation was meant to remove. **This is the same discipline as the label-every-outlet rule on [[12_Engineering_and_Systems/analysis/Switches_and_Controls|Switches and Controls]], applied to the low-voltage side.**
- **⚠️ The access-frequency corollary, from the same practitioner's own flat.** His hall picture conceals both the consumer unit and the low-voltage panel holding the LED drivers, hung on **two nails** — adequate precisely because «туда я вообще никогда не забираюсь». **But: «если бы здесь был ещё и СЧЁТЧИК, и мне нужно было постоянно туда лазить, то я бы просто запарился снимать и надевать эту картину. Если у вас там электросчётчики, то обязательно продумайте какие-то ПОВОРОТНЫЕ элементы — чтобы картина жёстко крепилась и ЛЕГКО ОТКРЫВАЛАСЬ.»** **→ The fixing method follows ACCESS FREQUENCY: drivers, never touched → nails; a meter, read regularly → a hinged panel.** [source: [[_Sources/YT_lhikl-7c43c_nsdsgn_own_flat_year_one_fixes|YT_lhikl-7c43c]]], [source: [[_Sources/YT_WCoqOCofPx4_nsdsgn_durable_interior_ten_rules|YT_WCoqOCofPx4]]]

**⚠️ Dusty shades measurably reduce output, and clear glass is the worst case.** Same practitioner, his previous flat: a black wire "spider" with glass bulb-shades. «**Примерно через пару МЕСЯЦЕВ они полностью покрываются пылью** — а у меня ещё кот, они становились такими МОХНАТЫМИ КОЛБАМИ, и их нужно каждый раз снимать, мыть. Если бы они были из ТКАНИ, из МАТОВОГО СТЕКЛА даже, их реже нужно было бы [мыть], потому что **на прозрачном стекле видно КАЖДУЮ ПЫЛИНКУ**.» A champagne cork eventually smashed one and he removed the rest: «стало более лофтово и гораздо симпатичнее. **СВЕТА ТОЖЕ СТАЛО ГОРАЗДО БОЛЬШЕ, потому что эти ПЫЛЬНЫЕ ПЛАФОНЫ очень сильно изолировали свет.**» **→ Two separate costs: a recurring cleaning obligation, and a progressive loss of output between cleans. Specify frosted glass or fabric where a shade is wanted at all** — which sharpens this page's existing "what to check before buying" list. [source: [[_Sources/YT_WCoqOCofPx4_nsdsgn_durable_interior_ten_rules|YT_WCoqOCofPx4]]]
...
#### ⚠️⚠️ Two dimming specifications that decide whether a scheme works at all — both discovered in occupancy

**Александр Синчуков, reporting on his own flat one year after moving in.** Both of these are component-level facts that never appear in a lighting specification and both defeat the design if got wrong.

1. **⚠️ THE DIMMING FLOOR — the bottom of the range, not lamp selection, is what delivers usable night light.** He has night-light recesses at the entrance and two in the corridor by the bathroom, fitted with Xiaomi units, and the reason he chose them is the floor: «они дали мне САМЫЙ НИЗКИЙ СВЕТ… регулировка идёт от **1 до 100%**, и я реально включаю **НА 1%**, и вот это **ЕДИНСТВЕННЫЙ вариант, который мне подходит по яркости**». **And the failed alternatives are named: «до этого я пытался какие-то САМЫЕ СЛАБЫЕ ЛАМПЫ купить, вставлять им ШТОРКИ, ТОНИРОВАТЬ — это всё давало НЕ ТО».** **→ Physical mitigations do not substitute for a driver that can actually reach 1%. Most cannot, and the datasheet range is therefore a hard selection criterion for any navigation or night fitting.** Cross-reference the night-and-navigation-light definitions on [[12_Engineering_and_Systems/analysis/Lighting_Design|Lighting Design]], where a dimmer is already characterised as a calibration device — **this supplies the number that calibration has to reach.**
2. **⚠️ THE MEMORY REQUIREMENT — a dimmed level must survive being switched off.** On his bathroom strip: «когда ты выключаешь надолго свет, он **НЕ СБРАСЫВАЕТСЯ**, потому что это **БЛОК С ПАМЯТЬЮ**». **→ Without memory in the driver or dimmer, every switch-on returns to full brightness and the calibrated scene has to be re-set by hand each time — which in practice means it stops being used.** Specify it explicitly; it is not a default.

[source: [[_Sources/YT_lhikl-7c43c_nsdsgn_own_flat_year_one_fixes|YT_lhikl-7c43c]]]
