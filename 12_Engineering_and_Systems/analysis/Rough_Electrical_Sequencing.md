...
- **⚠️ RECORD WHERE THE CABLES WENT — three tiers of solution from one channel, and the cheapest one costs a pencil (NSDSGN, 2022–2024).** This channel raises the same problem from three angles across three sources, so it is recorded here as one item.

  **The problem, stated flatly by a contractor: «я за всё своё время работы не сталкивался с реальной ситуацией, когда какой-либо исполнитель давал мне какую-либо исполнительную документацию. Никогда не было такого.»** Even where an electrical project exists, **there are always discrepancies and always additions** — the executor builds partly to the project and partly not, and then leaves. [source: [[_Sources/YT_Z0brwxSe7gQ_nsdsgn_engineering_systems_site_review|YT_Z0brwxSe7gQ]]]

  **⚠️ The measured cost of not doing it: a colleague's flat of 30–40 m² had the rough electrics run a year earlier; that electrician has since moved away; the replacement arrives and «вообще не знает, за что браться». On 30 m² an electrician spent TWO DAYS working it out. «А если квартира гигантская, 200–500 м²? Никак не разберёшь. Прозванивать можно — это время, это деньги» — and many simply say it is easier to re-run everything.** [same source]

  **⚠️ And the reason to treat it as a planning assumption rather than a precaution: «стройка часто идёт 8 [месяцев], если большой объект — то может год идти. Мы все живые люди: кто-то может заболеть, кто-то может переехать.» Personnel turnover across a year-long build is the expected case, not the exception.** [same source]

  **The three tiers, cheapest first:**

  1. **⚠️ Pencil-and-photograph, and any homeowner can do it alone: while the штробы are still open, write the dimensions on the wall in pencil — «от этого угла 50 см» — then photograph it. «И у вас будет КАРТА всех штроб вашей будущей квартиры.»** Because once the malyarka is done «вы не найдёте уже концов»; detectors get a mention, but **«эти фотографии будут ваши путеводные карты — по этой карте вы легко найдёте провода и никогда не попадёте в них саморезами».**
     **⚠️ And it extends past cables: photograph the ЗАКЛАДНЫЕ (plywood blocking) in plasterboard walls, and photograph the STUDS. A real current case: his tiler asked him to send a photo showing where the studs run so as not to hit them.** [source: [[_Sources/YT_2vyIWKmrSXM_nsdsgn_twenty_post_occupancy_regrets|YT_2vyIWKmrSXM]]]
  2. **Label everything, which costs a marker: all the junction boxes labelled AND the wires themselves labelled. «Даже после инженера, который начинал, придёт любой другой — он разберётся быстро.»** Plus a **технический проект** recording which luminaires, where they were bought and from whom — **and a resale argument: when selling you hand the buyer a folder, «вот папка, и в ней всё: как идут провода, как заложена сантехника, электрика».** [source: [[_Sources/YT_Z0brwxSe7gQ_nsdsgn_engineering_systems_site_review|YT_Z0brwxSe7gQ]]]
  3. **A 3D scan kept as a POINT CLOUD and deliberately NOT modelled — «ты не строишь модель, ты просто облако точек оставляешь… это стоит копейки».** Worth it particularly above ~200 m² where the runs are voluminous and cross each other. Recorded in full on [[11_Budget_and_Planning/analysis/Contract_Practice|Contract Practice]].

  **⚠️ And the free fallback if none of that was done: the installation rules themselves. Штробы must run VERTICALLY (per ПУЭ), and junction boxes under a stretch ceiling should sit no more than ~150 mm from the edge so local demolition can reach them — so a detector plus those two rules will locate most services.** **The catch is that this only works if a competent electrician did the work: with a developer's electrics «там вообще концов не найдёшь», and he describes seeing developer штробы run diagonally — «больше похоже на произведение современного искусства… очень эксцентричную штробу наискосок, чтобы потом вообще никто не понимал, чем он руководствовался».** [sources: [[_Sources/YT_SEdNCGA0Ttg_nsdsgn_choosing_builders_roundtable|YT_SEdNCGA0Ttg]], [[_Sources/YT_2vyIWKmrSXM_nsdsgn_twenty_post_occupancy_regrets|YT_2vyIWKmrSXM]]]

- **⚠️ Too few sockets, fifteen years on — and the stated cause is aesthetic, not budgetary (NSDSGN, 2022-10-25).** «Добавление двух-трёх, десяти-двадцати даже лишних розеток никак не изменит ваш бюджет, никак не изменит объём работ» — but once living there «каждый раз вы будете припоминать то решение: здесь, здесь и здесь вам не хватило всего лишь одной розетки». **The cause he names and attacks: clients think a socket is ugly — «как унитаз, почему-то все считают, что унитаз это нечто страшное и постыдное». He concedes the TV genuinely is ugly («просто чёрный квадрат») but not sockets — and expensive, genuinely attractive designer sockets exist.** *(Corroborates this channel's kitchen rule of a two-or-three-socket block per metre, and its kids-room finding that transformability is bought at rough stage in socket count — four sockets in one child's room specifically «чтобы с ростом ребёнка можно было легко делать перепланировку».)* [sources: [[_Sources/YT_2vyIWKmrSXM_nsdsgn_twenty_post_occupancy_regrets|YT_2vyIWKmrSXM]], [[_Sources/YT_2B67955_Z5I_nsdsgn_site_review_finish_details|YT_2B67955_Z5I]]]
