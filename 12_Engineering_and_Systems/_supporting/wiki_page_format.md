...
- **Keep the single guide page, but shrink each Core Technical Section to a short summary (2-5 bullets) + a link to a dedicated detail page** under that room/topic's own `analysis/` folder (e.g. `07_Bathroom/analysis/Bathtub_and_Shower.md`). The guide page becomes something a reader can scan top-to-bottom in a couple of minutes, not something they read start to finish. A section genuinely short enough already (one paragraph, no real depth — e.g. Bathroom's Ceilings section) can stay inline; splitting it out would just be an extra click for no benefit.
- **A flat "quick-reference Do's/Don'ts" table that previously lived as a *separate* file** (e.g. the old `07_Bathroom/analysis/Dos_and_Donts.md`) gets folded back into the guide page's own Quick Reference section instead — a separate file duplicating content already covered by the guide's narrative was the specific structural problem that motivated this whole change (two views of overlapping content, neither fully compact). One consolidated Quick Reference table per room, inside the guide page, per the original template's step 6 — not a second file.
...
- **Pilot status**: applied first to `07_Bathroom` (2026-08-17) as a trial before rolling out to other pages that have grown large (`12_Engineering_and_Systems/Plumbing_and_Waterproofing.md` at 354 lines and `13_Surfaces_and_Finishes/Doors_and_Trim.md` at 239 lines are the next candidates by size). Revisit this note once a second page has gone through the conversion, to confirm the pattern generalizes rather than being Bathroom-specific.
