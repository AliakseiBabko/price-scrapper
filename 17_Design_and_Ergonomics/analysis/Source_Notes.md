# Design & Ergonomics — Source Notes

Traceability record for `17_Design_and_Ergonomics`. Not reader content — kept off the main guide page by design.

| Source | Title/Topic | Channel | Feeds |
|---|---|---|---|
...
| `LB2oYxyYL3c` | Partition under an existing stretch ceiling, no embedded backing | Kupe-dsgn | Sliding Partition Mechanisms |
...
A 16-video batch dispatched specifically on combining colours in interior design. One video (`LRtedmEn8mQ`, Anuta Vlady) was already in the vault from 2026-08-25 and was not re-processed; one (`AQWuTQfr8w8`) was skipped by the value filter. Feeds the four colour pages: **CP&MD** = Colour Palette & Material Direction, **CH&CR** = Colour Harmony & Combination Rules, **CSP&T** = Colour Selection Process & Testing, **N&ETP** = Neutrals & Earth-Tone Palettes.

| Video ID | Content | Channel/Practitioner | Routed to |
|---|---|---|---|
| `jOXPHR9Mpek` | Two rules of colour combination — match on lightness and/or saturation, not hue; the fan-deck-row proxy | SHELNAT DESIGN / Наталья Шелковская | CH&CR |
| `ZY_Xp8b_G5M` | The seven named colour harmonies with per-scheme interior verdicts | Уроки рисования Artcolor.Online / Julia Omelchenko | CH&CR |
| `wjNz2wCtG7o` | Большой/малый контраст vocabulary, the "волчок" balance model, Soviet-interior nuance-matching caution | Андрей Самарин | CH&CR |
| `gEhVxuxtOjc` | Why a technically correct palette still gets rejected — образы среды / социальные коды / личная память; atmosphere→style→colour intake order | Дизайнер интерьера Надежда Кузина | CSP&T, CP&MD (Perspectives) |
| `n5ZBqdq0wH8` | Earthy palette in practice — five sub-families, the "случайный гость" test, lightness-spread over hue-count | Дизайнер интерьера Надежда Кузина | N&ETP, CSP&T |
| `_Vw-eaRekDk` | Three approaches to the main colour; selection-vs-distribution; family compromise; bold transit rooms / calm dwelling rooms | Бюро ARCHWOOD / Марина Измайлова | CSP&T, CH&CR, Whole-Apartment Coherence |
| `AKxOtRhIcJY` | Colour Q&A — ready-made palettes, wood as a palette member, ceilings, window orientation, "сложные" colours, composition self-checks | Бюро ARCHWOOD / Марина Измайлова | CSP&T, CH&CR, N&ETP, Curtains & Window Treatments |
| `jJ05zqugmSc` | Beige in depth — base-beige definition, the two paths, pairings; dark-room dissent; warm-white ceilings | Vittoria Selina / Вика | N&ETP, CP&MD (Perspectives), CSP&T, CH&CR |
| `PGrxKyLhxp8` | Six named colour failure modes — warm/cool and чистый/припылённый matching, large-block accent dilution, three accent-wall rules | Anuta Vlady | CH&CR, Decor & Finish Selection Technique |
| `zF1YLMqIvNw` | Beige vs. grey decision criterion; the fan-deck lightness correction; sample juxtaposition | Школа дизайна Маши Черной | N&ETP, CSP&T |
| `S9F9TGVG4x8` | 60/30/10 as hierarchy; the "якорный предмет" method; neutrals don't count against the three roles; 20–30% accent threshold | PRIDE / Алёна | CH&CR, CSP&T |
| `EipFPdVqNgs` | Muted-complementary rule; one colour on a grey base; five dated 2026 pairings | Студия Wall Street / Виктор | CH&CR (partial) |
