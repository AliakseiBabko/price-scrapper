...
async function main() {
  ...
  const url = `https://catalog.onliner.by/sdapi/catalog.api/search/washingmachine?mfr[0]=bosch&page=1`;
  ...
  try {
    ...
    if (data && data.products) {
      console.log(`Found ${data.products.length} Bosch washing machines on page 1:`);
      for (const p of data.products) {
        if (!p.full_name.toLowerCase().includes('сушильная')) {
