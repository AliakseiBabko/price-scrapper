...
## Approvals, Combined-vs-Separate, and How Many Bathrooms
...
### ⚠️ Number of Bathrooms — a Minimum Rule Plus a Real Case (Игорь Краснов, added 2026-09-01, Round 4)

Recommends a MINIMUM of two full bathrooms (or at minimum a separate WC/bath split) regardless of household size, for genuine independence between household members. **Real anecdote**: his own parents' country house, a 9-person multi-generational family gathering, initially had 3 bathrooms causing real queuing/conflict ("постоянные очереди... практически невозможно в этом существовать"), expanded to 5 bathrooms via a reconstruction (completed the prior summer) which solved it — a real, single-account planning case worth recording as concrete data alongside this page's own approvals/expansion-direction content above. `single-account`, `unverified`. [source: [[_Sources/YT_FIAZhQGKq7g_krasnov_7designer_taboos|FIAZhQGKq7g]]]
...
- **Shower-stall clearance must also be judged against the finished dimension, not the raw framing gap — two independent Category 5 worked examples, both flagging a defect at a similar range.** One case: 41cm raw clearance before wall-leveling/tile shrank to 35cm finished, judged unusable for the specific (larger-statured) occupants. A second, independent case: a 65cm-wide shower stall left the user's shoulders nearly touching both walls, with the source recommending roughly 80cm (~15cm more) as a comfortable minimum. `single-account` each, `unverified`. [sources: [[_Sources/YT_nNrZzbTpRi8_full_disaster_1.5m_282|YT_nNrZzbTpRi8]], [[_Sources/YT_cWRgenv4B40_full_disaster_30m_274|YT_cWRgenv4B40]]]
- **A master/ensuite bathroom's door should open directly from the bedroom it serves, not require passing through a shared/public space (e.g. the living room) to reach it.** A documented defect: a self-designed layout required leaving the bedroom and crossing three door openings including the living room to reach the "master" bathroom, defeating its privacy purpose — the fix was simply relocating the bathroom's entry point onto the bedroom wall. `single-account`, `unverified`. [source: [[_Sources/YT_nNrZzbTpRi8_full_disaster_1.5m_282|YT_nNrZzbTpRi8]]]
- **A raised shower platform (podium)'s height should be justified by an actual technical reason (plumbing/riser routing), not chosen arbitrarily** — a documented defect used a 30cm platform height with the plumbing stack routed directly behind adjacent tile, i.e. no routing reason for that much height. Verify a stated technical justification before accepting a raised platform's height. `single-account`, `unverified`. [source: [[_Sources/YT_cWRgenv4B40_full_disaster_30m_274|YT_cWRgenv4B40]]]
- **A door stopper/limiter's mounting distance from the wall must be checked against the room's actual passage width, not installed generically** — a documented defect had a rubber door stopper positioned far enough from the wall that the door would rebound off it directly into anyone entering a cramped WC, described as capable of striking a small child near eye level. `single-account`, `unverified`. [source: [[_Sources/YT_cWRgenv4B40_full_disaster_30m_274|YT_cWRgenv4B40]]]
- **A service/revision access panel's hinge/opening direction should be checked against the actual clearance available to operate it — a recurring finding across multiple independent sources now.** Documented at least three times in this store's Category 5 sources (two different apartments, two different practitioners' critiques) where a panel opened in a direction that made it difficult or impossible to actually use, despite technically "opening." `single-account` each, `unverified`, but now a repeated pattern worth treating as a standard install-QC check. [sources: [[_Sources/YT_78FeaeMZHOI_2.5m_thanks_developer_275|YT_78FeaeMZHOI]], [[_Sources/YT_cWRgenv4B40_full_disaster_30m_274|YT_cWRgenv4B40]], and the earlier Category 5 trial's `o7lGqaUuGm0`]

## Sizing, Clearances and Ergonomic Minima
...
- **Bathtub length matched to occupant height, per Zemskov**: length ≈ occupant height, or up to 10cm less — his worked example: a 178cm-tall person fits a 180cm tub comfortably, but the same person in a 190cm tub could slide under and choke. His defaults: 1700mm typical / 1800mm for taller occupants. **Same practitioner restates this identical figure and worked example in an earlier video** (`yt_S7v901mUACc`, #131, 2020-08-30, vs. this entry's own #145, 2021-08-22) — a same-channel repeat, not independent corroboration (per this vault's own corroboration rule), but confirms the rule is a stable, repeated standard for this practitioner rather than a one-off.
...
figures. [source: [[_Sources/YT_LvppSP_QUrg_laburo_ideal_layout_key_dimensions|YT_LvppSP_QUrg]]]
...
## Layout Decisions and Fixture Placement
...
blocking light entirely. `single-account`. [source: [[_Sources/YT_QginFVl00Hw_petrishin_nonstandard_bathroom_panoramic_vidnoe|YT_QginFVl00Hw]]]
...
## Cost and Finish-Thickness Budgeting
