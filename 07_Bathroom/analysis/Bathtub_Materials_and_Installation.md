# Bathtubs — Materials, Aprons and Installation Technique

Choosing a tub material and building it in: the apron/screen options, the revision hatch, tub-before-tile sequencing, silicone-joint and load-testing spec, and the podium. **Split out on 2026-09-02 under the vault's 300-line page ceiling, verbatim.**
...
### ⚠️⚠️ The Same Sag, With EPOXY GROUT Instead of Sealant — a Real Leak, in a Designer's Own Bathroom

**The section above establishes the mechanism (an acrylic tub sags when filled) and prescribes a thin SILICONE joint. This is the documented case of what happens when the joint is made with a RIGID material instead — and it is the strongest confirmation on this page that the material choice at that joint is load-bearing, not cosmetic.** Александр Синчуков, his own 4 m² bathroom. [source: [[_Sources/YT__nDCLhRUojE_nsdsgn_own_bathroom_4m2|YT__nDCLhRUojE]]]

- **The sequencing was correct** — tile brought down over the rim from above, «так мы получаем САМЫЙ ТОНКИЙ ШОВ», exactly as this page prescribes. **The material was not: the builders grouted that joint with EPOXY GROUT, the same as all the other tiling.**
- **⚠️ «Это была просто УЖАСНАЯ ОШИБКА, потому что ванна АКРИЛОВАЯ. Когда [она] набиралась, она ДЕФОРМИРОВАЛАСЬ — АКРИЛ ЭТО НЕ ЧУГУН, ОН ИМЕЕТ КАКОЙ-ТО ЛЮФТ. По периметру стали появляться тонкие противные ЩЕЛИ. ЭПОКСИДНАЯ ЗАТИРКА ТВЁРДАЯ, НЕ ЭЛАСТИЧНАЯ, и она стала ОТРЫВАТЬСЯ.»**
- **Then it leaked, slowly, and he deferred dealing with it: «когда ты принимаешь душ, вода стала потихонечку заливаться… в итоге однажды у меня СРАБОТАЛ ДАТЧИК ПРОТЕЧЕК. Я залез под ванну и увидел там ЛУЖУ ВОДЫ.»**
- **⚠️ THE FIX AND THE RULE: re-grouted with SEALANT. «ГЕРМЕТИК имеет очень БОЛЬШОЙ ХОД, он отлично СЖИМАЕТСЯ-РАЗЖИМАЕТСЯ, и проблема с этой щелью НАВСЕГДА ИСЧЕЗЛА. Поэтому ВСЕГДА, ЕСЛИ У ВАС АКРИЛОВАЯ ВАННА, ВСЕГДА ДЕЛАЙТЕ ЗАТИРКУ ГЕРМЕТИКОМ.»**

> [!IMPORTANT]
> **⚠️ WHY THIS IS WORTH ITS SPACE ALONGSIDE THE LOAD-TESTING RULE ABOVE.**
>
> **The load test protects against a seal broken ONCE, at first fill. This is a joint that fails CYCLICALLY** — the tub flexes on every fill and empty, and a rigid grout cannot accommodate repeated movement no matter how well the tub was loaded at installation. **Load-testing and the correct joint material are therefore two independent requirements, and satisfying one does not cover the other.**
>
> **⚠️ AND THE GENERALISATION IS WIDER THAN BATHS: a rigid grout cannot bridge a junction between elements that move relative to each other.** The tile-to-bath line is where every bathroom has one; a tile-to-shower-tray line and a tile-to-worktop line are others. **The tell is that the joint is between DIFFERENT MATERIALS, one of which deflects under service load.**
>
> **⚠️ Note also that a competent, epoxy-grouting tiling crew produced this: the error is not carelessness but the very natural instinct to finish the whole room in one material.** He accepts the supervision failure as his own: **«строители — я просто даже НЕ ОТСЛЕДИЛ этот момент, хотя на стройке надо всё отслеживать. ОБЯЗАТЕЛЬНО СЛЕДИТЕ ЗА ЭТИМ.»** → **A named, cheap, checkable inspection item for the wet-zone rough and finish stages — see [[11_Budget_and_Planning/analysis/Technical_Supervision|Technical Supervision]], where a fall is already recorded as the highest-value thing to supervise. This joins it: both are invisible once finished and both flood the room.**
>
> **This is also the FIFTH leak-sensor save recorded from this practitioner, and the only one where the sensor caught a defect in his own specification rather than a component failure — see [[12_Engineering_and_Systems/analysis/Leak_Protection_Systems|Leak Protection Systems]], where the installation that caught it is now itemised.**
...
Part of [[07_Bathroom/analysis/Bathtub_and_Shower|Bathtub and Shower]].
