# Bathroom — Shelving, Storage & Furniture
...
- **Tiled shelf labor-cost figure, new specific number (added 2026-08-28, Round 5)**: a single small shelf recessed into a wall and finished in tile costs roughly **5,000-12,000 RUB (≈$50-$130) in tile-setter labor alone** — a full day's work per shelf, before materials or surrounding niche wall-lining. The concrete cost reason behind this page's existing "tiled niches are the most expensive" framing. Konstantin Kruglov / Ontario, `single-account`, `unverified`. [source: [[_Sources/YT_kxr8zFvUTj8_kruglov_pinterest_bathroom_breakdown|YT_kxr8zFvUTj8]]]
- **Hidden/flush-mount shelving material — a third option beyond tile and glass, and a real installation-timing difference between all three (added 2026-08-24)**: Konstantin Kruglov / Ontario adds **metal** as a third hidden-shelf material (alongside tile and glass), rising in popularity specifically due to growth in online marketplaces and small local manufacturers producing this hardware. **The concrete practical difference between the three isn't durability or looks, but *when* each can be installed**: a tiled shelf/niche must be planned during the design and tiling stage — it cannot be added later. **Glass shelving and hidden-mount metal shelving can both be installed at any point after the renovation is finished, including 5-7+ years later** — genuinely retrofittable options if a tiled niche wasn't planned for up front. `single-account`, `unverified`. [source: [[_Sources/YT_IFnZxitFeNk_kruglov_bathroom_trends_2026|YT_IFnZxitFeNk]]]
...
- **Small-bathroom storage via a raised water-inlet node**: raise the water-inlet node ("узел приёма воды") on a platform, addressed at project stage, to create usable storage space underneath for toiletries/paper goods. `single-account`. [source: `_Archive/processed_sources/20260804_what_is_this_60m2_contractor_control_f7ab173e.txt`]
- **Floor-standing vs. wall-hung vanity cabinet — reconsider the default "wall-hung is always better" assumption**: wall-hung is easier to clean under, but if the sink sits close to the tub and you'd need to reach its own overflow/drain siphon for repair, a wall-hung cabinet can be very difficult to remove for access. A floor-standing cabinet with a countertop-mounted sink and a wall-mounted faucet lets you simply slide the cabinet aside for the same access.
- **The wall-hung-cabinet removal difficulty is a spectrum by cabinet width, plus a concealed-mixer mitigation (added 2026-08-28, Round 5)**: a narrow wall-hung vanity is difficult but still one-person-manageable to remove/reinstall for drain-overflow access; a wider, more massive cabinet needs real effort even for one person; the widest cabinets can genuinely need a multi-person team. **If the mixer is built-in/concealed rather than deck-mounted on the cabinet or sink itself, it never needs disconnecting during cabinet removal** — one fewer variable in an already-difficult job. Konstantin Kruglov / Ontario, `single-account`, `unverified`. [source: [[_Sources/YT_kxr8zFvUTj8_kruglov_pinterest_bathroom_breakdown|YT_kxr8zFvUTj8]]]
- **A niche cut between a toilet's installation frame and an adjacent plumbing-service cabinet functions as a dust trap, not real storage (added 2026-08-28, Round 5)**: this specific niche location must stay very shallow/low to preserve service access to the concealed utilities on both sides — too small to hold anything genuinely useful, so it ends up collecting dust rather than working as storage. `single-account`, `unverified`. [source: [[_Sources/YT_kxr8zFvUTj8_kruglov_pinterest_bathroom_breakdown|YT_kxr8zFvUTj8]]]
- **Two concrete new storage techniques (added 2026-08-28, Round 5)**: (1) a **side-loading concealed shelf** built into a furniture panel, loaded/accessed only from one side and fully invisible from the room's main entry sightline regardless of where in the room it sits — distinct from the sightline-placement rule above, which is about *where* an open niche sits, not a side-loading furniture mechanism; (2) **two smaller laundry baskets tucked under/beside a wall-hung vanity**, a space-saving alternative to one tall (~90cm) hamper when floor space for a full hamper isn't available. Konstantin Kruglov / Ontario, `single-account`, `unverified`. [source: [[_Sources/YT_kxr8zFvUTj8_kruglov_pinterest_bathroom_breakdown|YT_kxr8zFvUTj8]]]
...
the room is the overrated part. `single-account`, `unverified`. [source: [[_Sources/YT_oDHSbp6QRRE_kruglov_worst_overrated_trends|YT_oDHSbp6QRRE]]]
