...
**This topic is split across four pages** (2026-09-01, verbatim — one file had grown to 690 lines carrying four independent decisions):

- **This page** — *which tile to buy*: size and format taxonomy, porcelain quality parameters, undertone and relief, stone-look and style-group matching, buying practice and scam warnings.
- [[07_Bathroom/analysis/Tile_Installation_Sequencing_and_Acceptance|Tile Installation, Sequencing & Acceptance]] — *how it goes on and how to check it*: why tile falls off, laying sequence, corner and layout planning, podium and niche details, the acceptance checklist.
...
- **⚠️⚠️ AND THE HATCH MUST OPEN ALONG THE TILE JOINTS** — which constrains the hatch position and the tile setting-out together. Пальмира-Дом: the boxing frame and hatch are fitted before tiling so supervision can confirm access to taps, manifolds, filter, meter and heater, **and the tiler closes the frame only once the hatch is confirmed not to need correction**; the plumber must therefore allow for the future thickness of finishing leveller and tile when setting outlet heights. **⚠️ Use a RIGID profile for the frame so the hatch does not warp over time.**
  **→ ⚠️⚠️ A THIRD INDEPENDENT SOURCE for services being set by the tile grid** — after Огурцов above (mixers and sockets centred on tile or joint) and Шемчук (extract fan centred on a joint). **This one adds a requirement the other two do not: the hatch has to OPEN on joints, not merely sit on them.** [source: [[_Sources/YT_Y1auAmyqKo0_palmiradom_ppr_works_sequence|YT_Y1auAmyqKo0]]]
