...
- **Adhesive application technique, with tile-size thresholds**: any wall tile larger than 30×30cm needs adhesive combed onto the wall **and** a thin layer buttered directly onto the tile's own back (not just one side). Any floor tile larger than 60×60cm needs the same double-sided treatment. **Two named mistakes**: (1) old-style "dot/dab" application (a handful of adhesive blobs instead of a full combed bed) — explicitly called an obsolete practice that no longer works with modern tile/adhesive; (2) an adhesive layer applied too thick — adhesive always shrinks slightly as it cures, and a thick layer shrinks enough to significantly raise detachment risk.
...
- **Material cost (2025-08-08, trailing 6-month USD/RUB average 82.56)**: cement grout 350 RUB/2kg (under $10, not meaningfully roundable to this store's nearest-$10 convention at this magnitude); epoxy (two-component) grout ≈4,500 RUB/2.5kg (≈$50) — roughly an order of magnitude more expensive by weight even before accounting for the cheaper option's own USD figure being too small to round further.
- **Labor/installation cost, per running meter, same source and date**: cement grout 200 RUB/m (under $10); epoxy grout **from** 600 RUB/m (under $10) **and up** — the source gives no fixed ceiling for epoxy labor, only a floor, and both figures at this magnitude are left as their RUB originals rather than forced into a $10-bucket USD conversion that would round each to $0.
...
- **⚠️ Real worked cost comparison, standard vs. large-format wall tile**: for a 30m² bathroom wall area, standard 60×60cm porcelain tile (material + delivery + lift + install) totaled **≈180,000 RUB (≈$2,000, trailing-6-month USD/RUB average 90.5123 ending 2024-11-01)**, vs. large-format 80×160cm totaling **≈330,000 RUB (≈$3,600)** for the same area — a **≈150,000 RUB (≈$1,700)** premium attributable purely to tile-size choice. The source's own framing: on an ordinary bathroom this premium is largely wasted (much of the wall ends up hidden behind a mirror/shelving/fixtures regardless of tile size) — large-format wall tile only earns its premium in a genuinely large (10-15m²+) bathroom, typically in a house, where size itself is a deliberate status statement.
...
**Three layout approaches exist**: [source: [[_Archive/processed_sources/20260731_zems_b054_tiles_64563b4e.txt|zems_b054_tiles_64563b4e]]]
...
- **Two-tone large-format tile (120×60cm) color scheme**: darker tile lower, lighter tile upper on the same wall, for a deliberately "cozy" look. Quartz-vinyl flooring meeting tile in a doorway opening (no threshold strip) reads as seamless when done this way.
...
- **Format-size reference ladder**: floor baseline 60×60cm, up to 75×75/90×90cm in larger rooms, 90×120cm only with a large specialist tile-cutter (installer charges more); smaller rooms drop to 45×45 or 40×40cm. Wall tile: 60×60 or 60×30cm — a "universal" size specifically because it aligns with standard inspection-hatch mounting; any other wall format requires the design project to separately plan the hatch opening and plumbing behind it.
- **⚠️ Textured-tile fixture-fit risk, named design-oversight**: any fixture mounted against textured/patterned porcelain (waves, hexagons, fish-scale, stripes) — a socket/switch plate, a mixer's decorative escutcheon, a toothbrush-holder mount — won't sit flush against the wavy surface, leaving a gap silicone can't close; achieving a clean fit needs near-jewelry-precision work. Designers are stated to often overlook this when specifying textured tile.
- **Quality-factor taxonomy**: caliber (dimensional tolerance across pieces — higher quality tends toward zero tolerance; a cut-edge type lets the installer trim on-site for a premium flush result), edge type, surface type, pattern, and color-depth (through-body vs. surface-only).
- **Surface-type placement rule**: matte/structured only on floors (glossy is slippery and dulls over time); glossy is good on walls (reflective effect visually enlarges the room); lappato (part matte/part gloss) not recommended in a small room.
...
- **⚠️ Grey tile, especially in a dirty zone, reads as a permanently dirty floor.** She estimates ~90% of the grey tile on the market is an unhelpful shade for the interiors most people actually want. **The effect is strongest with concrete-look porcelain and worst when the tile is mottled** — in an entrance hall where dirty shoes stand, the brain reads the pattern as dirt rather than as texture, and the cost is the recurring urge to re-mop a clean floor.
- **⚠️ Grey mottled tile paired with heavily-figured knotty wood in a bathroom** is named as a specific texture-overload failure producing the same dirty reading; worst in small bathrooms.
