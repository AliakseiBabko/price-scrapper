...
def build(spec: dict, output: Path, manifest_path: Path) -> dict:
    # REFUSE A RETIRED SPEC. `current_apartment_base.json` was retired on
    # 2026-09-16: it is an early schematic - 18 walls against 25, two default
    # thicknesses against five measured ones, no shafts, and openings with no
    # provenance at all. The existing state is now generated by
    # `tools/ifc/model_from_dxf.py` from the gated DXF. Without this guard,
    # running build_variant.py would quietly regenerate the OLD model over the
    # new one, which is exactly the silent-divergence failure being fixed.
    status = str(spec.get("status", ""))
    if status.upper().startswith("RETIRED"):
        raise SystemExit(
            "refusing to build from a RETIRED spec (%s).\n"
            "The existing state is generated by tools/ifc/model_from_dxf.py from\n"
            "the gated DXF. See _Inbox/planning/model_rebuild_scope_20260916.md." % status)
