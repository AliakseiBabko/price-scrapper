...
All 16 are logged in `00_Master/processed_sources.csv` (`run_20260819_<video_id>` / `run_20260820_<video_id>`), have source extraction notes under `_Sources/` (except the confirmed duplicate and the 2 no-captions skips), are integrated into the intermediate store's Change Log, and were wiki-routed the same session as processed.
