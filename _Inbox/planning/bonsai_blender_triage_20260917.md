...
| **3** | **Concealed cable routes in 3D** (modelled run vs. zone) | ⚠️ **ANSWERED — OUTSIDE openBIM** (revised 2026-09-17) | Алексей Дубровин (`kxJ9R3gQjKw`), LOFT DIY (`0T97_CA7hgo`), Алексей Шемчук (`0c-QhBDQMWE`, vault prior) | ⚠️ **The void is real for openBIM/Bonsai and FALSE in general.** Three Russian practitioners model electrics — and plumbing — in 3D as ordinary commercial work, outside the IFC toolchain. Дубровин takes off **chase length by selection** (*«штроб на объекте 67 с лишним метров»*), a priced LABOUR item; Шемчук carries a per-cable schedule with start, end and length. **The benefits come from the GEOMETRY layer, not the system/port layer Lloyd Sark reports broken.** The 2D-overlay fallback is a labour-saving retreat in a HAND-modelling workflow, and that reason does not transfer to a pipeline that generates geometry. Full reconciliation in `00_Master/Model_and_Views.md`. |
