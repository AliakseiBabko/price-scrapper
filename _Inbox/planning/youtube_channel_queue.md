...
- **RemProektMD** — ACTIVE, trial in progress. See `remproektmd_channel_plan_20260824.md`. **Scope flag**: this is a Moldova (Chisinau) company channel, a new country/currency for this project — no MDL rate support in `currency_converter.py`, so MDL prices are recorded but not USD-converted; technique content is unaffected. Worth the user's attention as a genuine scope question, not something to assume approval on indefinitely.
