...
**Tilt-and-turn is unambiguously the better default over turn-only, all else equal** — the cost premium is small (~1-2% of total window cost), negligible against the functional gains. Three advantages:
...
**The problem**: standard safety-code parapet height (~1.1m) conflicts directly with a panoramic-window design goal — at that height, someone seated sees mostly the opaque parapet and only a strip of sky, not the view a floor-to-ceiling glazed section is meant to provide.

**Standard resolution**: split the window into a fixed panoramic (non-opening) section below the code-required parapet height, and a small opening transom (фрамуга) above it, rather than making the whole panoramic window openable.
