# Kitchen — Appliances & Fixtures (Non-Cabinetry)

First content on this page (previously an empty placeholder). Covers appliance/fixture selection distinct from [[03_Kitchen/Kitchen_Furniture|Kitchen Furniture]]'s cabinetry/assembly/countertop content.

## 2026 Trend Appliances (added 2026-08-28)

> [!NOTE]
> From this channel's three 2026-trends videos (Round 8, videos 3-5) — cross-checked against each other for overlap before recording; the mixer-brand comparison is stated almost verbatim in two of the three sources and recorded once here. [sources: [[_Sources/YT_SaMpFOPm_4U_kruglov_kitchen_trends_2026|YT_SaMpFOPm_4U]], [[_Sources/YT_f3EI72Nwemk_kruglov_best_kitchen_ideas_2026|YT_f3EI72Nwemk]]]

- **⚠️ Steam-function oven** — a scaled-down home version of a professional combi-steamer (the type used in high-end restaurant kitchens), available at mid and premium price tiers (mid-tier is a real but compromised option; premium has no compromises). Named benefits: juicier/softer food, prevents over-drying, faster cooking, better baking, and — the source's own strongest claim — excellent specifically for *reheating* (a reheated slice of pizza reportedly comes out tasting/feeling freshly baked, versus visibly worse texture from a microwave).
- **⚠️ Integrated-hood cooktop (venting built into the cooktop), a premium-segment answer to the no-upper-cabinets trend** (see [[03_Kitchen/analysis/Furniture_Facade_Materials|Furniture Facade Materials]] for that trend): once upper cabinets — and the wall that would normally carry a hood — are removed, an integrated-hood cooktop solves the "where does the hood go" problem. Requires the ventilation duct route planned well in advance, most often under the screed; these units are physically larger than a standard cooktop, so unsuitable for a small kitchen. **Named manufacturers**: Bora, Bosch, Miele, Smeg, Siemens, Elica.
- **Food-waste disposer/grinder ("диспоузер")** — installs directly under the sink drain and grinds food waste before it reaches the trash. Benefits: eliminates odor buildup from trash sitting a day or two; reduces sewage-clog risk (waste is ground fine, not whole); genuinely useful during vegetable prep (peelings go straight down the drain); modern units run quietly. Activation: a water-flow sensor triggers grinding automatically, with a manual override button to force it off.
- **⚠️ Two-in-one kitchen-mixer brand comparison, first named-brand mixer/faucet content for this store**: **Amikiria/Amikiriant** is the year's best-selling combined kitchen-faucet-plus-drinking-water-tap model with a pull-out flexible spout, but the source's own subjective critique flags two usability drawbacks — (1) re-docking the pull-out spout needs precisely finding a narrow metal alignment tube; (2) no stream-pattern adjustment. **Two named alternatives the source personally prefers**: **Aelsberg Venta (model IXF)** and **Sancas Mola/Imola** (Italian/Russian-branded, likely China-manufactured per the source's own observation) — both dock via magnet (no precise alignment needed) and both offer stream-pattern adjustment, useful for rinsing delicate produce. Explicit non-sponsorship disclaimer from the source.

`single-account`, `unverified`. [sources: [[_Sources/YT_SaMpFOPm_4U_kruglov_kitchen_trends_2026|YT_SaMpFOPm_4U]], [[_Sources/YT_f3EI72Nwemk_kruglov_best_kitchen_ideas_2026|YT_f3EI72Nwemk]]]

## Combined Oven+Microwave Unit — Small-Apartment Space-Saving Option (Kruglov/Ontario, Round 14 triage, added 2026-08-28)

**⚠️ A single appliance housing both an oven and a microwave**, saving the floor/wall space a separate built-in or countertop microwave would otherwise need. **Real tradeoff**: while the microwave function is in use, the oven function is unavailable (shared internal space/mechanism) — a genuine functional compromise, not just a footprint saving. **Cost note**: the combined unit costs roughly the same as a standalone oven alone — the added microwave function doesn't meaningfully raise the price. `single-account`, `unverified`. [source: [[_Sources/YT_EwI_ZoT3VTQ_kruglov_top11_small_apartment|YT_EwI_ZoT3VTQ]]]

## Sink & Mixer Color Caution: Avoid Black (added 2026-08-28)

> [!NOTE]
> From the "useless kitchen items" checklist video. [source: [[_Sources/YT_e3bHUlP0PMY_kruglov_useless_kitchen_items|YT_e3bHUlP0PMY]]]

- **⚠️ Avoid black sinks and black mixers/faucets, specifically for the Russian domestic-water context**: Russian tap water commonly carries mineral content (iron, etc.) that leaves visible mineral-deposit residue once the fixture air-dries; a black finish only looks clean immediately after hand-drying and reads as visibly dirty the rest of the time. Recommend a different (non-black) sink/mixer color for this reason.

`single-account`, `unverified`. [source: [[_Sources/YT_e3bHUlP0PMY_kruglov_useless_kitchen_items|YT_e3bHUlP0PMY]]]

## Heated-Floor Thermostat Placement and Other Small Fixtures (added 2026-08-28)

> [!NOTE]
> From a real single-project kitchen walkthrough. [source: [[_Sources/YT_9aVNKzaxGSI_kruglov_top_solutions_modern_kitchen|YT_9aVNKzaxGSI]]]

- **⚠️ Avoid mounting a heated-floor thermostat on the kitchen backsplash** — the backsplash is a kitchen's highest-demand real estate for outlets/small-appliance placement, and a heated floor has a real 2-3 hour thermal lag before warmth is felt anyway, so instant backsplash access provides no practical benefit. **Mount the thermostat on a wall away from the backsplash instead**, and use the thermostat's twice-daily schedule-programming capability rather than relying on immediate manual access. See [[07_Bathroom/analysis/Heated_Floor_and_Thermostat|Heated Floor and Thermostat]] for this store's general heated-floor/thermostat content.
- **Hidden sponge-holder mount**: a sponge holder/clip mounted to the underside/front face of the sink lip itself, keeping it out of sight while still within reach.

`single-account`, `unverified`. [source: [[_Sources/YT_9aVNKzaxGSI_kruglov_top_solutions_modern_kitchen|YT_9aVNKzaxGSI]]]

## "Whole Truth About Kitchens 2026" — Sink Mounting, Mixer Spray Modes, Appliance Budgeting (added 2026-08-28)

> [!NOTE]
> From this channel's densest kitchen video to date (~75 min) — cross-checked against this page's existing sink/mixer/appliance content and this round's own videos before recording anything below. [source: [[_Sources/YT_FdJLbYEpViU_kruglov_whole_truth_kitchens_2026|YT_FdJLbYEpViU]]]

- **⚠️ Sink-mounting three-tier framework, extending this round's own rimmed-sink caution with the full comparison**: **top-mount/rimmed (drop-in)** — cheapest, widest material/style range, fits any countertop, least modern, hardest to keep clean at the rim. **Undermount (glued from below, sometimes with added mechanical fixings)** — modern, easiest to sweep debris straight into the basin, but **only compatible with a countertop material that doesn't need edge-banding at the cutout** (rules out LDSP; works with quartz/acrylic/composite). **Fully integrated (monolithic, typically acrylic)** — easiest to clean, best aesthetics, highest cost.
- **Sink shape preference reinforced across a third independent source**: round sinks are the clear "outsider" (can't properly wash a baking tray/rack); rectangular or square is the only recommended shape.
- **⚠️ Mixer spray-mode taxonomy**: **aerator** (mixes air into the stream, reduces water use); **jet/stream** (denser, higher-pressure, for scrubbing heavily soiled items); **spray** (rinsing plates/produce); **mist/fog** (delicate produce, avoids bruising); **pulse** (alternating drip/jet, super-premium). A flexible pull-out spout is called "the one correct type" for a kitchen sink mixer; a built-in drinking-water outlet on the main mixer itself (not a separate tap) is called the ideal configuration.
- **⚠️ Food-waste disposer price: ≈30,000-40,000 RUB (≈$370-490)** — the source's own stated range and explicit recommendation to always install one if budget allows.
- **Sink dual-basin/1.5-basin configuration** — a real configuration choice (one larger + one smaller basin) alongside a standard single-basin sink.
- **Named additional-tier appliances not previously recorded on this store's kitchen pages**: dish/plate warmer (подогреватель посуды); ice generator (ледогенератор); built-in kitchen scale (встроенные кухонные весы, premium-only).
- **⚠️ Appliance-budget framing, converted example**: cabinetry and appliance costs are separate, comparably-sized budget lines — the source's own worked example: **≈300,000 RUB (≈$3,700)** cabinetry paired with **≈1,000,000 RUB (≈$12,400)** of appliances for the same kitchen — budget for appliances as a distinct, potentially larger line item, not an afterthought.
- **Service-tier distinction**: economy-tier kitchens are typically self-service (modular/prefab order, DIY assembly, or a high-volume showroom order); comfort/premium include a dedicated personal design/technical manager guiding the full specification.

`single-account`, `unverified`. [source: [[_Sources/YT_FdJLbYEpViU_kruglov_whole_truth_kitchens_2026|YT_FdJLbYEpViU]]]

## Price Data Points: Disposer, Flexible-Spout Mixer, Integrated Sink (2023 vs. 2025, added 2026-08-28)

> [!NOTE]
> From a 2023 real-project video by a different on-camera presenter (Nikita Kuznetsov) on the same Ontario channel — earlier price points than this round's own `FdJLbYEpViU` (2025), recorded as a price-evolution data point across roughly two years rather than a disagreement. [source: [[_Sources/YT_-1HBQkULK4Y_kruglov_top10_popular_kitchen_mistakes|YT_-1HBQkULK4Y]]]

- **⚠️ Food-waste disposer price at time of this (2023) recording: starting from ≈10,000 RUB (≈$110)** — considerably lower than this round's own `FdJLbYEpViU` (2025 recording, ≈30,000-40,000 RUB/≈$370-490) — a plausible price-rise data point across roughly two years, not a disagreement.
- **⚠️ Flexible-spout mixer price: starting from ≈4,000 RUB (≈$40)** — a concrete entry price point for this store's existing flexible-spout-mixer recommendation.
- **⚠️ Integrated same-material sink price add-on: ≈25,000 RUB (≈$270)** added to the countertop cost for a seamless same-material integrated sink.

`single-account`, `unverified`. [source: [[_Sources/YT_-1HBQkULK4Y_kruglov_top10_popular_kitchen_mistakes|YT_-1HBQkULK4Y]]]

## Refrigerator Selection (see dedicated page, added 2026-08-28)

> [!NOTE]
> This channel's general refrigerator buying-guide content (types, cooling systems, compressor
> economics, price/brand tiers) is routed to the dedicated, previously-empty
> [[15_Appliances/Kitchen_Refrigerators|Kitchen Refrigerators]] category page rather than
> duplicated here — that page already existed as a reserved category-page stub for exactly this
> kind of content. [source: [[_Sources/YT_FmGVmt2RH1c_kruglov_which_refrigerator_2026|YT_FmGVmt2RH1c]]]

## Dishwasher Selection (see dedicated page, added 2026-08-28)

> [!NOTE]
> This channel's general dishwasher buying-guide content (width standards, wash-quality
> drivers, drying types, price/brand tiers) is routed to the dedicated, previously-empty
> [[15_Appliances/Kitchen_Dishwashers|Kitchen Dishwashers]] category page rather than
> duplicated here. This store's existing dishwasher *placement/sizing-in-context* content
> (the 45cm small-kitchen compromise, sink-adjacency rules) stays on `Kitchen_Furniture.md` —
> distinct scope from this buying-guide content. [source: [[_Sources/YT_10sNVkAEATw_kruglov_how_to_choose_dishwasher_2026|YT_10sNVkAEATw]]]

## Oven Selection Criteria (added 2026-08-28)

> [!NOTE]
> This channel's first dedicated general oven buying-guide video for this store — cross-checked
> against this page's and `Kitchen_Furniture.md`'s existing oven/microwave content before
> recording. [source: [[_Sources/YT_AOlNxAlI0So_kruglov_how_to_choose_oven_2026|YT_AOlNxAlI0So]]]

- **Exclusions**: gas ovens (worse temperature precision, fewer modes, worse safety); 45cm
  "compact" ovens (a compact oven's usable area is roughly an A4-sheet footprint — in ~90% of
  cases loses badly to a full-size 60cm model); freestanding (non-built-in) ovens (don't fit a
  modern integrated kitchen — only suit retro-style kitchens or niche projects).
- **Heating-mode taxonomy**: top heat / bottom heat / combined top+bottom (the base mode,
  covering ~70% of tasks); convection (internal fan, even heating); "enhanced"/professional
  convection (fan plus a heating ring behind it, reaches target temperature faster — ideal for
  meat); grill (browning, meat).
- **⚠️ Microwave-function critique, with the underlying mechanism**: the "avoid microwaves"
  health trend is overstated (an intact-housing microwave doesn't harm health per measurements
  cited), but reheated food comes out drier/worse-textured because a microwave heats *water
  molecules* in the food, not the food evenly — different foods heat unevenly since they hold
  different water content (a meat patty heats faster/hotter than potato on the same plate, even
  rotating). **A combined oven+microwave unit has a real functional tradeoff**: you can't run
  the oven at full capacity while the microwave function is active — recommends splitting into
  two separate appliances if you use the oven often, but still endorses a combined unit for a
  small studio/one-bedroom apartment. See the `[!WARNING]` combo-unit disagreement on
  [[03_Kitchen/Kitchen_Furniture|Kitchen Furniture]] for how this relates to this store's
  existing two-sided callout on combining oven and microwave.
- **⚠️ Steam-function three-tier taxonomy**: (1) simple one-time water-pour humidity boost —
  uncontrolled, evaporates almost immediately, explicitly called useless in practice (a named
  competitor example, Kuppersberg, cited as a poorly-implemented case); (2) automatic
  continuous water feed — meaningfully better, sustains humidity but without fine humidity-level
  control; (3) full combi-steam with 100% steam control — precise temperature *and* humidity
  control throughout cooking, the actively recommended tier. Benefits: reheated
  chicken/pizza retain just-cooked texture/moisture (vs. a microwave's dry result);
  dough rises better with a crisper crust; meat/fish retain moisture with a browned crust;
  whole-dish reheating (e.g. lasagna) heats evenly rather than burning the top while the
  center stays cold. Tradeoff: full-steam reheating takes ~10+ minutes vs. ~1-2 minutes in a
  microwave (some models reach target chamber temperature in 3-4 minutes via the
  professional-convection heating ring).
- **Sous-vide oven mode**: a restaurant-derived low-temperature (50-75°C) vacuum-sealed cooking
  technique (an hour to a full day depending on the dish); an oven with a built-in sous-vide
  function automates the temperature/steam-level maintenance a manual water-bath setup needs.
- **⚠️ Named budget-tier "steam-blocking" feature, undocumented by manufacturers**: some
  budget ovens can't implement real steam generation but instead block steam/moisture from
  escaping the chamber — a worthwhile addition even at budget price. Named example: Elica
  implements this on budget-tier models; not marketed by the manufacturer, known mainly to
  specialist installers.
- **Controls**: mechanical / electronic-touch / hybrid (most common — mechanical dials plus
  some touch buttons). Wi-Fi/app-connected ovens explicitly not recommended — often useless
  or forgotten, less convenient than mechanical; "simpler is better and more reliable."
- **⚠️ Four-tier cleaning taxonomy, with a real recurring cost flagged**: manual wipe-down;
  hydrolysis (a marketing term for a grease-resistant wall coating — works "so-so," now
  near-universal rather than a real differentiator); catalytic panels (grease-decomposing
  cassette liners) — **need replacement after 2-3 years**, a real recurring maintenance cost;
  pyrolysis (door locks, chamber heats to 500°C, burns grease to ash/dust) — most effective.
  **Pyrolysis and 100%-steam currently can't coexist in one unit** — pick one; the source
  recommends prioritizing 100% steam, since a steam oven's humid environment also keeps grease
  from baking onto the walls in the first place, partially offsetting the lack of pyrolysis.
- **Door construction**: soft-close hinge (safer/more convenient); glass-pane count (2/3/4 —
  more panes = cooler exterior glass = safer around children/pets), with a caveat that some
  2-pane and 4-pane models perform similarly despite the difference (undisclosed in spec
  sheets — ask a specialist if this matters).
- **⚠️ Telescopic rails/runners**: add **≈2,000-5,000 RUB (≈$30-$60)** to the unit price —
  worth it for pulling a tray/dish out to a stable, level position. **Hand-wash only, never
  dishwasher** — high-pressure jets can force water past internal seals and jam the mechanism.
- **⚠️ Price/brand tier ladder, this store's first oven brand-tier ladder**: **economy
  ≈30,000-50,000 RUB (≈$380-$630)** — a single dominant Chinese factory (Midea) supplies a
  large share of this tier, rebadged under many brands (Mefery, Hbeg, MEA, Coopersberg, Lex,
  History, among ~50 companies per the source). **Comfort ≈70,000-100,000 RUB (≈$890-$1,300)**
  — framed as "loaded Chinese-made" vs. "bare-bones European-branded" at the same price; named
  brands Teka (acquired by Midea; ~6-month Russia supply gap at filming time), Bosch,
  Electrolux, Elica. **Named example**: an Elica oven (made at the Italian IVA factory) at
  ≈130,000 RUB (≈$1,600) comes fully loaded (tinted glass, steam-blocking, telescopic rails,
  soft-close door, full touch controls, professional convection, 7-year warranty) — **the same
  factory-made unit under the IVA brand costs roughly twice as much for identical spec**.
  **Premium: 100%-steam ovens start at ≈300,000 RUB (≈$3,800)** — named brands AEG, SMEG, Teka,
  Neff. A second named IVA/Elica price-doubling example: Elica's 3-in-1 oven+steamer+microwave
  unit sells for ≈280,000 RUB (≈$3,500) under the Elica brand vs. ≈550,000 RUB (≈$7,000) for
  the identical factory unit under the IVA brand — the source states it cannot explain the
  mechanism behind this roughly 2x brand-name price gap.

`single-account`, `unverified`. [source: [[_Sources/YT_AOlNxAlI0So_kruglov_how_to_choose_oven_2026|YT_AOlNxAlI0So]]]

## Kitchen Electrics — Outlet Placement Technique and the Norms That Actually Exist (Надежда Кузина, added 2026-08-31)

A ~47-minute recorded seminar by interior designer Надежда Кузина, most of it spent **disagreeing with the socket-layout diagrams that dominate search results**. Heights are on [[12_Engineering_and_Systems/analysis/Mounting_Heights_and_Positioning|Mounting Heights & Positioning]]; lighting is on [[12_Engineering_and_Systems/analysis/Lighting_Design|Lighting Design]]. [source: [[_Sources/YT_iHViNm3dESU_kuzina_kitchen_electrics_and_lighting|YT_iHViNm3dESU]]]

> [!IMPORTANT]
> **Jurisdiction: Russia.** The normative claims below cite Минэнерго's ПУЭ and ГОСТ/СНиП. They are recorded here, on the technical page, **not** in `16_Legal_and_Regulations/`, which is Belarus-only. Do not treat any of them as applicable to this apartment without checking the Belarusian equivalent.

- **⚠️ The most useful finding is a negative one: she searched for binding rules on kitchen outlet *positions* and found almost nothing.** "Очень мало чего касается конкретного расположения." The ГОСТ/СНиП documents cited in socket articles mostly require that kitchen outlets be earthed and that three phases be available — not where they go. **Most placement "rules" circulating online are convention, not code.**
- The placement rules she *could* substantiate (RU): **no outlet within 50 cm of a gas pipe** (ПУЭ); **minimum one outlet per 3 m of room perimeter** (an old Soviet-era design norm); **no outlets under or over the sink**; and wet-zone zoning carried over from bathroom ГОСТ putting outlets in zone 3 — **≥60 cm from the sink**, with the same 60 cm applied to the hob.
- **⚠️ The under-sink prohibition collides with reality and she resolves it explicitly.** Nearly every kitchen now has a waste disposer («измельчитель»), and with appliances left and right there is often no neighbouring base to move to. Of the two available options — sink zone or plinth — **she chooses the sink zone**, because routing the disposer cord to the plinth leaves it hanging and deliberately bent round. **Two conditions: splash-proof («влагозащищённые») outlets, mounted as high in the base as possible** to reduce flood exposure. She pairs this with a standing default of a leak sensor at every wet point.
- **Every outlet position behind cabinetry is inconvenient — the choice is which inconvenience.** Plinth = lie on the floor and reach; behind the appliance = pull the appliance out; behind a neighbouring cabinet = unload the shelf first. **The mitigation that makes any of them acceptable is a separate breaker («автомат») per appliance**, so power can be cut without physically unplugging.
- **Appliance cords are short — microwaves worst of all**, and this drives outlet migration more than aesthetics does. **Practical hedge: add a spare outlet in the plinth behind the kitchen furniture** so there is somewhere to reach when a cord falls short.
- **Floor-standing appliances (dishwasher, washing machine) can have nothing behind them** — the outlet must be adjacent. She specifically rejects diagrams putting a dishwasher outlet under the sink when it could share a double with the oven next door.
- **⚠️ Extractor hood — the one hard error: never put the outlet on the hood's centre axis.** The ducting («гофра») runs down that centreline and collides with it. Built-in hood → outlet behind the element it is built into; free-hanging hood → outlet inside the casing («кожух») so it is invisible.
- **⚠️ Her method for a hood not yet chosen, and it generalises**: leave an **electrical stub («вывод») rather than a flush-mounted outlet**; once finishes are done and the hood physically arrives, fit a **surface-mounted («накладная») outlet** that hides inside the casing. Rationale: casings differ in their fixings, and a stub can be nudged to clear them where a set flush box cannot. She notes the real sequencing behind it — nobody chooses the hood right after the walls go up; everyone runs off to pick sanitaryware first.
- **Consolidate controls into one frame** — she singles out diagrams scattering the worktop-light switch separately across the backsplash. Put it on the same line, in the same frame, as the outlets.
- **Sequencing rule the whole talk rests on**: builders demand electrics almost as soon as the walls are up, so **the kitchen layout and the specific appliances must already be decided by then**. "Чем меньше вы знаете, тем больше вероятность, что придётся переносить." **But she is deliberately un-dogmatic about the consequence** — moving an outlet *behind* cabinetry later is genuinely survivable, since that wall is never seen. The rule bites on visible outlets and on appliance-dependent geometry. **Work from the wall elevation («развертка»), not the plan.**
- **With a glass backsplash («скинали») there is no such tolerance** — cut-outs are fabricated into the glass and nothing can shift afterwards.
- **Pop-up outlets in the worktop**: showroom furniture. Defensible only for something used very rarely; anything permanently plugged in leaves the column standing up — worse-looking than an ordinary backsplash outlet, and unhygienic.
- **Servo-driven fittings must be in the electrical drawing**: sensor bins and powered upper-cabinet lift-ups each need a stub **and** a hidden home for the driver block. She uses a dedicated bay, the same approach as the neighbouring-element outlet.

## Source Notes

- **Как правильно сделать электрику и освещение на кухне** (2021-09-26, added 2026-08-31, Кузина Round 1 video 1) — [[_Sources/YT_iHViNm3dESU_kuzina_kitchen_electrics_and_lighting|extraction note]]. Heights, lighting and cabinetry content cross-routed to `12_Engineering_and_Systems/` and `Kitchen_Furniture.md`.
- **10 Key Kitchen Trends for 2026** (2026-01-23, added 2026-08-28) — [[_Sources/YT_SaMpFOPm_4U_kruglov_kitchen_trends_2026|extraction note]].
- **The Best Kitchen Ideas of 2026: Top 10 Tricks** (2026-01-04, added 2026-08-28) — [[_Sources/YT_f3EI72Nwemk_kruglov_best_kitchen_ideas_2026|extraction note]].
- **Top Solutions for a Modern Kitchen** (2025-01-24, added 2026-08-28, Round 9 video 3) — [[_Sources/YT_9aVNKzaxGSI_kruglov_top_solutions_modern_kitchen|extraction note]].
- **ВСЯ ПРАВДА О КУХНЯХ 2026** (2025-09-13, added 2026-08-28, Round 9 video 4) — [[_Sources/YT_FdJLbYEpViU_kruglov_whole_truth_kitchens_2026|extraction note]].
- **TOP 10 Popular Kitchen Renovation Mistakes** — disposer/mixer/sink price data points (presenter Nikita Kuznetsov, 2023-11-08, added 2026-08-28, Round 9 video 5) — [[_Sources/YT_-1HBQkULK4Y_kruglov_top10_popular_kitchen_mistakes|extraction note]].

- **How to Choose an Oven in 2026** (2026-03-13, added 2026-08-28, Round 10 video 2) — [[_Sources/YT_AOlNxAlI0So_kruglov_how_to_choose_oven_2026|extraction note]].
- **Which Refrigerator Should You Buy in 2026?** (2026-02-27, added 2026-08-28, Round 10 video 3) — [[_Sources/YT_FmGVmt2RH1c_kruglov_which_refrigerator_2026|extraction note]]. Full content routed to `15_Appliances/Kitchen_Refrigerators.md`; only a cross-reference pointer kept here.
- **How to Choose a Dishwasher in 2026?** (2026-02-20, added 2026-08-28, Round 10 video 4) — [[_Sources/YT_10sNVkAEATw_kruglov_how_to_choose_dishwasher_2026|extraction note]]. Full content routed to `15_Appliances/Kitchen_Dishwashers.md`; only a cross-reference pointer kept here.

## Change Log

- 2026-08-28 — Page populated for the first time (was an empty placeholder) with appliance/fixture-selection content from Round 8's three 2026-trends videos, distinct from `Kitchen_Furniture.md`'s cabinetry/assembly content.
- 2026-08-28 (later) — Added "Oven Selection Criteria" section from Round 10's dedicated oven
  buying-guide video (mode taxonomy, steam-function tiers, cleaning methods, and this store's
  first oven brand/price-tier ladder); cross-referenced a third mechanism into
  `Kitchen_Furniture.md`'s existing oven+microwave-combo disagreement callout.

## ⚠️ Metal-Finish Coordination Is a Procurement Problem Before It Is a Design One (Надежда Кузина, added 2026-09-01)

A kitchen worked example, and the general rules are on [[17_Design_and_Ergonomics/analysis/Combining_Finishes_Wood_Metal_and_Gloss|Combining Finishes — Wood, Metal and Gloss]]. **Nobody sets out to mix six metals — it happens as a cascade**: buy a gold pendant before choosing the kitchen → pick facades with a gold profile to match → standard hardware has no gold, so buy gold handles separately → **but the built-in appliances have silver handles, and gold-handled built-in appliances are very hard to source**, so settle for black → then find a mixer roughly matching the facade handles. **Assembled, nothing matches**: in daylight the "gold" pendant reads almost silver, the mixer is matte, the handles glossy. [source: [[_Sources/YT_LAuZhM9tJWA_kuzina_combining_metal_finishes|YT_LAuZhM9tJWA]]]

**The specification limit worth carrying into an appliance schedule: built-in appliance handle finishes are a hard constraint, not a preference** — decide the kitchen's metal hierarchy before ordering the pendant, and check what handle finishes actually exist in the appliance range you want.
