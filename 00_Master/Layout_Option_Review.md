...
3. **✅⚠️ THAT BLOCKER IS CLOSED — `v0` HAS GEOMETRY as of 2026-09-10, and a NEW one replaced it.** The partitions were reconstructed from the vector plan into `data/canonical/v0_*_extracted.json` / `v0_named_walls_placed.json`, exported to `data/cad/dxf/v0_developer_layout.dxf`, and the DXF closure gate passes on it. **So a measured layout comparison is now available — `tools/layout/compare_variants.py` runs and produces the A3 sheet.** ⚠️⚠️ **The replacement blocker is a REVIEW, not a build: `v0_named_walls_placed.json` says `status: "DRAFT - not owner-reviewed."`, and it is authoritative for POSITION and FACES only.** Promoting draft geometry into the sheet the layout decision is read off is exactly what `tools/cad/PROVISIONAL_MODEL_POLICY.md` exists to prevent. *(Superseded text: this item read "`v0` HAS NO GEOMETRY… the single task standing between this project and a measured layout comparison".)*
...
> [!IMPORTANT]
> **✅ UPDATE 2026-09-11 — BOTH TASKS ARE DONE, AND THE CALLOUT ABOVE WAS RIGHT.**
> The вентблок was measured on **2026-09-04**; `v0` geometry was built on **2026-09-09/10**. **Neither needed a round of research, and three further rounds were run after this was written** — two video batches and two Gemini reports — **none of which moved this decision.** The callout's own claim, that the blocker was measurement rather than sources, held exactly.
>
> ⚠️ **What actually remains is smaller than either**: the owner reviewing the DRAFT v0 extraction, and two defects in the comparison sheet that were fixed on 2026-09-11 — a daylight rule that reported four rooms dark off an empty `space_boundaries`, and phase counts that printed `0 / 0` where cap2 is simply unbuilt. Both now report *not evaluable* and *не смоделировано* instead of a confident wrong answer. Seeds in `scripts/compare_variants_selftest.py`.
