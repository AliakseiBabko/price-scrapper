...
| Item | Status |
| :--- | :--- |
| **⚠️⚠️ THE MODEL'S STOREY HEIGHT DISAGREES WITH THE SETTLED CEILING HEIGHT.** `storey_height_m` is **2.8 m** in `current_apartment_base.json`, `current_apartment_shell.json` and `current_apartment_cad.json`, and it drives **every wall in every variant**. This page records, settled 2026-09-04, that **the clear height is 2500 mm screed-to-ceiling** (slab-to-slab ~2550) — so the model is **~250–300 mm too tall**. Found 2026-09-16 while adding the ceiling slab, which is placed at `storey_height_m` and therefore inherits the error. ⚠️ **Not fixed in that change, deliberately: it moves every wall, every drawing and every quantity, and the DXF closure gate is pinned to the current geometry.** **Consequence to know about meanwhile: the walkable view feels ~300 mm roomier than the flat will.** ⚠️ **And per this page's own note, quantities need TWO heights anyway (2500 to screed for plaster, ~2480 less skirting for paint), so the fix is not a single find-and-replace.** | **Open since 2026-09-16** |
