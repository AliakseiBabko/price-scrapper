...
- **The external insulation is a CONSTRUCTION FACT, not a junction mechanism, and it is not a modelled layer.** The owner instructed on 2026-09-08 that insulation must not be carried as a separate layer, because it may be removed or left in place. On 2026-09-09 he also described the corners where the south external wall meets the concrete columns as *"filled with the insulation, another layer which lays on top of the external walls."* Both are true and they do not combine into a closure directive: `data/canonical/junction_directives.csv` carried `J_MB_R2_R9` claiming those corners close by insulation infill, and it was **withdrawn 2026-09-09** because they close by masonry — R8 and R9 each span MB's full band and `check_dxf_closure.py` asserts both corner squares solid. A directive exempts the gate from checking a junction, so claiming an infill where there is no void made the gate weaker for nothing. **Insulation belongs in quantities and the envelope build-up (≈375–380 mm above), never in the junction ledger.**
