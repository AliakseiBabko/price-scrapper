...
| **⚠️⚠️ A BIGGER WINDOW IN THE 9.36 ROOM, AND MOVING ITS RADIATOR — raised 2026-09-18, NOT a decision.** Owner: *"I'm not sure about window in a smaller bedroom. Probably I would like to replace it with something bigger and move heat radiator a little bit to the side. This is what I have in my head. This is a possibility."* **What the model already knows.** The 9.36 room's window is **O4a**, and it does **NOT face the street** — it opens onto the **лоджия**. O4 is ONE 1380 mm opening in **MA**, split by a frame mullion into a ~600 window leaf and a ~770 full-height door leaf; MA's printed chain is 1120 + 1380 + 150 + 175 = 2825, so there is **1120 mm of solid wall west of the opening and 325 mm east**. MA is **300 mm autoclaved aerated block and the developer's own description calls the external walls «не несущие»** — non-load-bearing infill in a monolithic RC frame — so widening is a lintel question, **not a frame question**. ⚠️ But MA is the **thermal boundary**: the лоджия is recorded as THERMALLY DECOUPLED from the flat, so more glass in MA is more heat loss, and `O4a` has **a panel radiator under its sill** (sill 735). ⚠️⚠️ **THE PART THAT CHANGES EVERYTHING, AND IT IS THE RADIATOR, NOT THE WINDOW.** `building_spec.json` → `energy_supply`: *"Postanovlenie 164 section 6 puts a job in the project + tekhnicheskie usloviya track for GAS or CENTRAL HEATING. The gas limb is now permanently irrelevant, so **the ONLY thing that can flip this project into the designed-project track is touching central heating — i.e. moving a radiator.**"* **So this one casual idea is the single recorded action that changes the regulatory track of the whole renovation** — from a notification job to a designed project with technical conditions and a five-party acceptance. **And no height is recoverable from the floor: the screed is laid with the horizontal heating distribution in it.** **Nothing here says do not do it.** It says the radiator and the window are two decisions, not one, and only the radiator carries that consequence — the window may well be enlargeable while leaving the radiator where it is. **Needs the owner, and needs it before `Окна, откосы, подоконники` and `Отопление`/`Радиаторы` are scoped.** | **OPEN, raised 2026-09-18** |
