...
- **✅⚠️⚠️ THE FLAT IS BEING COMPLETELY REWIRED. Owner, 2026-09-17.** *«the current plan is to replace all the existing wiring. This is the current placement of electric outlets and switches — it is just for reference, for starting, for testing how to construct electric wiring.»*
  - **Everything surveyed and migrated so far is the `existing` phase — reference, not the design.** It stays in the model because it is a useful check that a reading of a photograph is plausible, but **no existing socket position constrains the new layout.**
  - **The planned topology is fixed:** a central board **near the entrance** → distribution through the **ceiling** → a **drop down the wall** to each accessory → **drops exclusively in aerated-block walls**, chased and covered by the finish.
  - **⚠️ This makes the new layout GENERATABLE, which it has never been.** It derives from appliance and fixture placement — the electric stove, the lighting fixtures — plus the topology above. The owner will supply the actual positions.
  - **⚠️⚠️ It deliberately writes off fitted work, and that should be costed, not discovered.** The handover (above) includes **electrics, sockets and switches already fitted and priced into the flat**, and this page warns that moving a socket wall throws away work already paid for. A full rewire discards that on purpose. **That is the owner's call and he has made it** — but it belongs in the budget as a known write-off.
  - Cast-in wiring is ordinary at pour time and is how the **existing ceiling outlets sit inside the slabs**; a retrofit cannot do that, which is why every new drop must land on block. Full rule and its correction: `data/canonical/wall_materials.json` → `A_CABLE_CANNOT_BE_CHASED_INTO_CONCRETE_2026_09_17`.

- **✅⚠️⚠️ NO NATURAL GAS IN THE BUILDING AT ALL — electric stoves only. Stated by the owner, 2026-09-08.** This was an open question as recently as the same day, and it settles more than it looks:
  - **✅ It unblocks the kitchen-into-living move.** СН 3.02.01-2019 **§4.8** restricts a living room over or under a **GAS-STOVE** kitchen. With no gas the clause cannot bite, so joining the 5.24 m² кухня to the living room is a **согласование question, not a prohibition** — which is what `v1` already does, and what **all four** of the designer's variant plans for this exact type start from (see `data/layout_cases/dubravinsky-euro3-designer-variants.json`). **A fourth independent argument for that move**, alongside the windowless kitchen and the two already recorded above.
  - **✅ The project track now hinges on ONE thing.** № 164 §6 puts a job in the **проект + технические условия** track for **gas or central heating**. The gas limb is permanently irrelevant here, so **the only thing that can flip this project into the designed-project track is touching central heating — i.e. moving a radiator.** That was always the sharper of the two risks; now it is the only one.
  - **⚠️ And it shifts the whole cooking load onto the electrical figure.** § 4 prohibits exceeding the premises' **calculated engineering load**, and with no gas the hob and oven sit on it alongside the appliance set. **Worth obtaining the design figure before the appliance order is placed** — a number to obtain, not a permit to apply for. See [[16_Legal_and_Regulations/analysis/Renovation_Permits_and_Approvals|Renovation Permits and Approvals]]. ✅ **Much less pressing than it would otherwise be, because тёплый пол is out of scope** (decision of 2026-08-26, below) — underfloor heating is normally the load that breaks the budget in an all-electric flat, and it is not being installed.
  - **❓ And it raises a genuinely open question about the acceptance commission.** № 164 §9 and the acceptance procedure both list **the energy AND gas supply organisations** among the attending parties, **unconditionally on the track**. With no gas connection there may be no such counterparty — which would cut the coordination from **five parties to four**. **Not confirmed: the text does not condition the list on whether the building has gas.** Worth asking the исполком, because it is a scheduling difference in the administrative month.
...
- **✅⚠️⚠️ THE WALL MODEL IS COMPLETE AND OWNER-CONFIRMED — `data/canonical/wall_materials.json`.** Established 2026-09-04 from the owner's own colour markup plus the developer's building description. **It is NOT derivable from the drawing: the plan uses the same hatch and the same thickness for concrete and for aerated block**, so this is owner-supplied evidence that cannot be recovered later.
  ...
  - **✅ M6b at 200 mm is OWNER-CONFIRMED, 2026-09-10** — it was provisional pending the owner's other plan, and it is not. ⚠️ **The drawing still disagrees and that is recorded, not reconciled**: no 200 mm hatched solid exists at that position (the vector shows 300, 150 and 100 there), so the owner is the source for this figure, the same standing as `wall_materials.json`. M6b is no longer quarantined and no longer excluded from quantities or IFC — see `data/canonical/wall_placement_directives.csv` (P_M6b). ⚠️ **Still provisional: the G2/G3/G5 thicknesses.** ⚠️ **And it cannot be enforced yet** — these ids are regions on a raster, not named shell walls, so mapping them needs **v0 geometry**, which remains the blocking task.
...
- → **It also answers the question left open by the case-study round: this is the decision in that case study the owner rejects.** The bunk bed and the themed mural remain his to state. **✅ UPDATE 2026-09-16: the BUNK BED half is now stated — no bunk beds. The themed mural remains open.**
...
**No underfloor heating** — user decision, 2026-08-26. Recorded as `our_scope: out_of_scope` in `data/deliverable_templates/price-scrapper-target-set.json`, which drops the target album from 16 sheets to **11**. **✅ Restated by the owner 2026-09-08 — «we don't plan to create any underfloor heating» — after I had written it back into a live design-load consideration two lines above. Corrected there, on the permits page, in the change log, in `data/canonical/building_spec.json`, and in `data/procurement/trade_requirements.json`, whose text is SENT TO CONTRACTORS and now says explicitly that тёплый пол must not be offered or priced.**
...
**⚠️ One consequence is NOT yet decided, and it is a material choice.** The vault carries a sourced claim that **a heated floor is a precondition for a barefoot porcelain or stone floor**, not an upgrade — so declining underfloor heating argues for a warmer floor material in the ванная, or for accepting a cold one. **The three options are laid out on [[07_Bathroom/analysis/Bathroom_Design_and_Palette|Bathroom Design & Palette]]**; a heated wall does *not* substitute, since it is capped near 27 °C. Separately, the same source names heating and ventilation as *jointly* necessary against mould, so **the drying duty now falls entirely on ventilation plus a towel warmer** — which matters more than usual in a single-aspect flat with no cross-ventilation.
...
| ✅ **weight is not a problem** | owner, 2026-09-06 — there is a concrete slab underneath and it will carry it |
| ⚠️⚠️ **the DESIGN must be repeated** | the contract with the developer allows replacing the glazing **provided the initial design is reproduced**: **four bays across × two rows = eight panes, dark frames**. So the division is a *specification*, not a preference. Our own basic plan draws three intermediate mullions between two end posts — **four bays confirmed from our own drawing**, at ~705 mm each over ~2.9 m |
...
**⚠️⚠️ SUPERSEDED 2026-09-07 — the target is now a FULL ALBUM, not an 11-sheet planning project.**

The owner adopted the 32-sheet album he supplied (Проект №5443509, Вероника Романова) as the model for
the final package: *"развёртки нужны обязательно... this is a full set of documents which will be in the
final portfolio."* The Dolgushev album that set the earlier target was, in his words, *"just an example
and probably not full."*

**New spec: `data/deliverable_templates/zk-dubravinskiy-full-album.json` — 31 sheets in scope**, against
11 before. What changed and why it matters:

| change | why |
| :--- | :--- |
| **развёртки стен back in — 9 sheets** | the largest change, and the most consequential: **развёртки are where socket and switch HEIGHTS get specified** rather than merely noted on a plan |
| **кондиционирование back in** | he raised AC routes on 2026-09-04, and the concrete cannot be cut |
| **канализация split from водоснабжение** | two independent stacks warrant their own sheet |
| **выключатели split from розетки** | as the reference does |
| **new: остекление лоджии** | not in the reference, but needed — the replacement is **contractually constrained to reproduce the original pattern** |
| **тёплый пол stays out** | the only excluded sheet, per the 2026-08-26 decision below |

Three prototypes already exist in that style: `_Drawings/sheets/sheet_01_sockets.png`,
`sheet_02_lighting.png`, `sheet_03_plumbing.png`, generated by
`tools/layout/sheets/make_services_sheets.py`.

> [!IMPORTANT]
> **This roughly triples the drawing work**, and `00_Master/Sheet_Production_Roadmap.md` is now sized for
> the wrong target. The capability analysis in the superseded
> `price-scrapper-target-set.json` is still the right way to *sequence* production — it is only the sheet
> list that changed.
...
- **⚠️ REVERSED 2026-09-10 — THE EXTERNAL INSULATION IS NOW A DRAWN LAYER.** The owner instructed on **2026-09-08** that insulation must not be carried as a separate layer, because it may be removed or left in place. On **2026-09-10**, looking at the vector-extraction render, he reversed it: *"still that M2 is not touching MA. See the gap. There shouldn't be gaps. There should be an insulation layer, which is outside — 70 millimetres thick. I want you to draw this external insulation layer instead of leaving the gap."* **The reversal also explained the gap exactly**: MA records 70 mm and its masonry starts at y 9050.6; M2 records 0 and its solid ends at 8980.6; 9050.6 − 8980.6 = **70.0 mm**. M2 never butted MA's masonry — it butts MA's insulation. It was a missing element, not a defect. **Which face the insulation sits on is taken from the drawing, not from a heuristic** (a flat-centroid rule gets M6b wrong): `tools/layout/place_insulation.py` accepts either a composite hatched solid whose thickness equals masonry + insulation (S22 = R8 250 + 150; S31 = R9 250 + 150 — both had been sitting in the "unused" pile looking like noise) or a perpendicular solid stopping exactly `insulation_mm` short of the face. **5 of 6 bands are settled that way; M6b is not settled by either test and is NOT guessed** — consistent, since the drawing does not contain M6b's 200 mm masonry either. Drawn on layer `V0-INSULATION`. ⚠️ The **withdrawn** `J_MB_R2_R9` directive is not revived by this: those corners still close by masonry, and the insulation is a layer rather than a junction mechanism.
...
| Item | Status |
| :--- | :--- |
| **✅ STOREY HEIGHT SET TO THE CONSTRUCTOR'S CLEAR HEIGHT — 2.5 m, 2026-09-16.** The model ran on **2.8 m**, which `tools/cad/build_base_spec_from_cad.py` listed under **`ASSUMED`** — *"not derivable from a 2D plan, stated not hidden"* — so this filled a placeholder rather than overturning a decision. **2.50 and not 2.55, because the field means what the WALLS span: screed top to ceiling underside, i.e. the CLEAR height.** The constructor's **~2550 slab-to-slab is a different measurement** and would have silently added the ceiling slab's thickness, which the model now carries separately. ⚠️ **The owner's photo readings of a comparable flat ("a little above 2500, tolerance unknown") CORROBORATE the constructor without refining him** — a photo with no independent scale reference cannot discriminate 2500 from 2550, per `Evidence_Reading_Discipline.md`; what it does do is rule out a materially different figure. **Applied to all three canonical files, both variants rebuilt, and the three geometry gates re-run and passing.** ⚠️ **STILL OPEN, and unchanged by this: quantities need TWO heights, not one** — 2500 to the screed for plaster, ~2480 less skirting for paint — so a single-height model is still wrong for one of the two trades. | **Height closed 2026-09-16; the two-heights quantity split still open** |
...
| **✅ `v0` GEOMETRY EXISTS — CLOSED 2026-09-11, and the row below is kept for the record.** `data/canonical/v0_wall_runs_extracted.json`, `v0_named_walls_placed.json`, `v0_openings_placed.json`, `v0_insulation_placed.json` and `v0_elements_extracted.json` were built 2026-09-09/10 from the vector plan, and `data/cad/dxf/v0_developer_layout.dxf` passes `tools/layout/check_dxf_closure.py`. ⚠️ **But `v0_named_walls_placed.json` carries `status: "DRAFT - not owner-reviewed."` and is `authoritative_for` POSITION and FACES only — `clear_mm`/`solid_mm` remain the length of record.** **So the geometry exists and is gated; what is NOT done is the owner's review of it**, and until that happens it must not be promoted into the variant comparison that drives the layout decision. *(Superseded text: the row previously read "`v0` HAS NO GEOMETRY… Open, and it BLOCKS layout selection", which stopped being true on 2026-09-10 and was still being quoted on 2026-09-11.)* | **Geometry closed 2026-09-11; owner review OPEN** |
...
| **✅ THE BUNK-BED QUESTION IS CLOSED — NO BUNK BEDS, owner-stated 2026-09-16.** *"My wife doesn't like bunk beds, so it looks like this is not an option for our kids' room."* **Recorded as a household PREFERENCE, which is what the open item was waiting for — it was never going to be settled by more sources.** ⚠️ **It happens to agree with everything the vault already held**, so nothing is being traded away: the clinical source argues against bunks *where area permits two separate beds* and 15.28 m² does; the shared configuration has a foreseen end at Phase 2; and two furniture makers reject them on footprint and on permanent visibility. **The agreement is worth stating explicitly because it means the preference costs nothing.** → See the consequences below. **The THEMED-MURAL half remains open** and is still the owner's to state. | **Bunk half closed 2026-09-16; mural half open since 2026-09-04** |
| **✅ CLOSED 2026-09-19 — NO WIDER WINDOW IN THE 9.36 ROOM, AND THE RADIATOR DOES NOT MOVE.** Owner: *"for now let's not explore wider window in the 9.36 room. We don't need Радиаторы and Окна, откосы, подоконники."* **⚠️⚠️ THE CONSEQUENCE IS REAL BUT I DESCRIBED IT WRONGLY — CORRECTED 2026-09-19 AFTER ANTIGRAVITY CAUGHT IT.** What is true: moving a radiator was the single remaining action that could have put this job in the **проект + технические условия** track under № 164 §6, and not moving it closes that exposure. **WHAT IS FALSE, AND IT WAS MY WORDING, NOT THE VAULT'S: I called the result a «notification track». THERE IS NO NOTIFICATION TRACK IN BELARUSIAN LAW.** That is a Moscow/Russian concept — work done first and reported after — and importing it is exactly the jurisdiction error standing rule 4 exists to prevent. `16_Legal_and_Regulations/analysis/Renovation_Permits_and_Approvals.md` is accurate and says the opposite: **§7 is a СОГЛАСОВАНИЕ / РАЗРЕШЕНИЕ track — a PRIOR PERMIT.** Partition changes and openings in non-load-bearing walls need исполком согласование but **NOT a designed проект**; §9 requires a pre-work **inspection** with the housing, energy and gas organisations present; §10 gives the исполком **up to one month** to decide, and the разрешение itself sets the completion and acceptance dates. Afterwards §17–21 still require an acceptance commission and a new technical passport. **⚠️⚠️ SO NO WORK MAY START BEFORE THE PERMIT IS ISSUED.** Beginning demolition on the belief that this is a notify-afterwards regime would be самовольная перепланировка under §22, exposing the owner to fines, a paid техническое заключение and a §28 restitution order. **Avoiding the radiator saved a designer's fee. It did not exempt anything from prior approval.** `building_spec.json` → `energy_supply` records that, with gas permanently irrelevant in this building, **moving a radiator was the single remaining action that could flip the job into the designed-project track** under Постановление 164 — technical conditions, a five-party acceptance. No wider window → no reason to re-centre the radiator → central heating untouched → that exposure is now closed rather than merely unresolved. ⚠️ **NOTHING WAS LOST WITH THE TWO SHEETS.** Reveals and sills go under `Отделка стен` and `Штукатурка`; radiators stay in `Отопление`; the лоджия glazing replacement keeps its own sheet `Остекление лоджии (замена)`. All four are `in_scope`. ⚠️ **What the model established and is worth keeping** even though the option was not taken: O4a opens onto the **лоджия, not the street**; the opening could only ever have grown **WEST**, into 1120 mm of non-load-bearing aerated block, because **east stops after 150 mm at R8, which is concrete frame**; and growing west at the same sill would have left the radiator untouched anyway. *(Superseded text: the row previously read "raised 2026-09-18, NOT a decision… Needs the owner".)* | **Closed 2026-09-19** |
| **⚠️⚠️ THE 9.36 ROOM HAS NO DIRECT OUTDOOR AIR PATH — raised by Antigravity 2026-09-19, REPRODUCED, and never previously confronted.** `boundary_face_inventory.csv` classifies exactly ONE face of that room's walls as `envelope`: **`MA.cross_lo`, «facade wall, insulated outward face»**. R6, G4a, G4d, G8 and R4 are all unclassified. **And MA's outward face gives onto the ЛОДЖИЯ, not the street.** So a through-wall breather on MA would draw supply from an enclosed, unheated buffer — one this project intends to seal with replacement triple glazing to hold it frost-free without heating, and which also carries laundry humidity and storage. That is re-breathing the buffer, not fresh air. ⚠️ **`Fresh_Air_Ventilation_and_Ducting.md` listed 9.36 among the three rooms that could take a through-wall unit. That line is now corrected** — the room does not qualify. ⚠️⚠️ **The room with the single worst acoustic adjacency in the flat** (`Soundproofing.md`: the neighbour's habitable 14.64 faces it) **is therefore also the one with no outdoor air**, and BOTH occupancy phases put someone sleeping in it — adults in SC-NOW, a child in Phase 2. **Routes that remain, none costed or chosen:** carry an intake across the loggia and penetrate `O9` or `M2`; duct a supply through the 2500 mm ceiling; or accept openable loggia glazing in winter, which contradicts the frost-free target. **This is a hard constraint on any layout that puts a bedroom in 9.36, and it belongs in the first coupled design slice.** | **OPEN, raised 2026-09-19** |
