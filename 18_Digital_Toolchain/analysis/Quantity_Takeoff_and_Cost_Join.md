...
- **⚠️⚠️ AND THE DEFECT IS IN THEIR SCHEMA: the line carries a region and NO DATE FIELD.** Region is first-class; the year is never mentioned. **A user of that tool cannot tell how stale a rate is, which is the entire purpose of carrying the date. → Copy the shape, add the field.**
- **⚠️⚠️ AND A DISTINCTION WORTH STATING ONCE HERE, BECAUSE IT GENERALISES TO EVERY SOURCE IN THIS VAULT: “the source states no date” IS NOT “the price is undated”.** **The publication date bounds every figure a source shows** — this demonstration was uploaded **2026-04-24**, so its catalogue is priced **no later than April 2026**. **Record that as an UPPER BOUND, never as an as-of date**, since a vendor catalogue can be stale behind its own demo. **An upper bound is enough to pick a historical FX rate and enough to compare against another dated figure**, which is what standing rule 2 asks for. → **What actually blocks transfer here is the REGION axis: US, Canada, India and Spain, and no Belarus.**
