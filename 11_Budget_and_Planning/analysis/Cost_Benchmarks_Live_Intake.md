# Renovation Cost Benchmarks — Live Intake Ledger

Real, dated cost benchmarks and worked cases gathered from processed sources, kept as an
append-only ledger. **Split out of `11_Budget_and_Planning/Budgeting_Guide.md` on 2026-09-02,
verbatim** — these sixteen entries had accumulated as `###` subsections under that guide's
"4. What to Check Before Estimating", which is not what they are.

> [!IMPORTANT]
> **Every figure here carries its own date, currency, region and delivery model, and they are
> not interchangeable.** A turnkey figure and a labour-only figure describe different scopes;
> so do a rough-stage case and a full-realisation one. **Read
> [[11_Budget_and_Planning/Budgeting_Guide|the Budgeting Guide]]'s delivery-model and
> price-per-m² sections before comparing any two entries below.**

Companion pages: [[11_Budget_and_Planning/analysis/Budget_Tiers_Cheap_Optimal_Premium|Budget Tiers]]
...
## Real object cases — moved to their own page

The individual real-object cases, the running project-cost tracking updates and the flood-remediation data point are now on **[[11_Budget_and_Planning/analysis/Cost_Benchmarks_Real_Object_Cases|Cost Benchmarks — Real Object Cases]]**. This page keeps the market-tier and rate benchmarks plus the budget-as-percent-of-purchase-price heuristic.
